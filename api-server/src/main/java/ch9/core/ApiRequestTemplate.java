...
public abstract class ApiRequestTemplate implements ApiRequest {
	...
	public void requestParamValidation() throws RequestParamException{
		...
		for(Object item : this.getClass().getClasses()[0].getEnumConstants()) {
			RequestParam param = (RequestParam) item;
			if(param.isMandatory() && this.reqData.get(param.toString()) == null) {
				throw new RequestParamException(item.toString() + " is not present int request param.");
			}
		}
	}
